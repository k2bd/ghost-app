...
const rowEntries = (moves: Move[], rowIndex: number): (string | null)[] => {
    const rowValues = moves
        .filter((move) => {
            move.position.y === rowIndex;
        })
        .sort((a, b) => a.position.x - b.position.x);

    const rowIndexes = rowValues.map((move) => move.position.x);

    const rowWidth = rowValues[rowValues.length - 1].position.x;
...
const colEntries = (moves: Move[], colIndex: number): (string | null)[] => {
    const colValues = moves
        .filter((move) => {
            move.position.x === colIndex;
        })
        .sort((a, b) => a.position.y - b.position.y);

    const colIndexes = colValues.map((move) => move.position.y);

    const colWidth = colValues[colValues.length - 1].position.y;
...
const ChallengeResponseDialog: React.FC = () => {
    ...
    const enabled = game.challenge?.move.playerName === player?.name;
    ...
    return (
        <Dialog isOpen={enabled} title="Challenge Response">
